...
class NowPlaying(Scene):
    def __init__(self, sonos):
        ...
        self.previous_button = Button(Rect(65, 370, 40, 40), image=previous_track_img,
                                      disabled_image=previous_track_disabled_img)
        # Touch Handler
    ...
    def zone_state_changed(self, data):
        """Callback function that is called every time the zone state changes ex. new track, play, pause,
        volume change, etc. """
        # print("")
        # pprint(data)
        # print("")          
        ...
        # avTransport data
        if 'current_transport_actions' in data: self.update_available_actions(
            data['current_transport_actions'].split(', '))
